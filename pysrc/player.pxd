...
IF CYTHON_FIGHTER_PLAYER_PXD == 0:
    ...
    cdef class Player(Object):
        ...
        def __init__(self, AnimData animation_data, AnimData outline_data):
            super().__init__()
            # self.animation = Animation(texture_path, 8, 1000 // 8)
            self.animation = Animation(animation_data)
            self.outline = Animation(outline_data)
        ...
        cpdef void draw(self, screen):
            ...
            self.outline.draw(screen, self.physics.position.x, self.physics.position.y, flipx=self.physics.orientation.x)
